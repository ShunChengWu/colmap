...
#include "photometric_bundle_adjustment.h"
...
////////////////////////////////////////////////////////////////////////////////
void PBAPrintSolverSummary(const ceres::Solver::Summary& summary);

ceres::LossFunction* PhotometricBundleAdjustmentOptions::CreateLossFunction() const {
...
bool PhotometricBundleAdjustmentOptions::Check() const {
...
PhotometricBundleAdjustmentConfig::PhotometricBundleAdjustmentConfig() {}

size_t PhotometricBundleAdjustmentConfig::NumImages() const { return image_ids_.size(); }

size_t PhotometricBundleAdjustmentConfig::NumPoints() const {
  return variable_point3D_ids_.size() + constant_point3D_ids_.size();
}

size_t PhotometricBundleAdjustmentConfig::NumConstantCameras() const {
  return constant_camera_ids_.size();
}

size_t PhotometricBundleAdjustmentConfig::NumConstantPoses() const {
  return constant_poses_.size();
}

size_t PhotometricBundleAdjustmentConfig::NumConstantTvecs() const {
  return constant_tvecs_.size();
}

size_t PhotometricBundleAdjustmentConfig::NumVariablePoints() const {
  return variable_point3D_ids_.size();
}

size_t PhotometricBundleAdjustmentConfig::NumConstantPoints() const {
  return constant_point3D_ids_.size();
}

size_t PhotometricBundleAdjustmentConfig::NumResiduals(
...
void PhotometricBundleAdjustmentConfig::AddImage(const image_t image_id) {
  image_ids_.insert(image_id);
}

bool PhotometricBundleAdjustmentConfig::HasImage(const image_t image_id) const {
  return image_ids_.find(image_id) != image_ids_.end();
}

void PhotometricBundleAdjustmentConfig::RemoveImage(const image_t image_id) {
  image_ids_.erase(image_id);
}

void PhotometricBundleAdjustmentConfig::SetConstantCamera(const camera_t camera_id) {
  constant_camera_ids_.insert(camera_id);
}

void PhotometricBundleAdjustmentConfig::SetVariableCamera(const camera_t camera_id) {
  constant_camera_ids_.erase(camera_id);
}

bool PhotometricBundleAdjustmentConfig::IsConstantCamera(const camera_t camera_id) const {
  return constant_camera_ids_.find(camera_id) != constant_camera_ids_.end();
}

void PhotometricBundleAdjustmentConfig::SetConstantPose(const image_t image_id) {
  CHECK(HasImage(image_id));
  CHECK(!HasConstantTvec(image_id));
  constant_poses_.insert(image_id);
}

void PhotometricBundleAdjustmentConfig::SetVariablePose(const image_t image_id) {
  constant_poses_.erase(image_id);
}

bool PhotometricBundleAdjustmentConfig::HasConstantPose(const image_t image_id) const {
  return constant_poses_.find(image_id) != constant_poses_.end();
}

void PhotometricBundleAdjustmentConfig::SetConstantTvec(const image_t image_id,
...
void PhotometricBundleAdjustmentConfig::RemoveConstantTvec(const image_t image_id) {
  constant_tvecs_.erase(image_id);
}

bool PhotometricBundleAdjustmentConfig::HasConstantTvec(const image_t image_id) const {
  return constant_tvecs_.find(image_id) != constant_tvecs_.end();
}

const std::unordered_set<image_t>& PhotometricBundleAdjustmentConfig::Images() const {
  return image_ids_;
}

const std::unordered_set<point3D_t>& PhotometricBundleAdjustmentConfig::VariablePoints()
    const {
  return variable_point3D_ids_;
}

const std::unordered_set<point3D_t>& PhotometricBundleAdjustmentConfig::ConstantPoints()
    const {
  return constant_point3D_ids_;
}

const std::vector<int>& PhotometricBundleAdjustmentConfig::ConstantTvec(
    const image_t image_id) const {
  return constant_tvecs_.at(image_id);
}

void PhotometricBundleAdjustmentConfig::AddVariablePoint(const point3D_t point3D_id) {
  CHECK(!HasConstantPoint(point3D_id));
  variable_point3D_ids_.insert(point3D_id);
}

void PhotometricBundleAdjustmentConfig::AddConstantPoint(const point3D_t point3D_id) {
  CHECK(!HasVariablePoint(point3D_id));
  constant_point3D_ids_.insert(point3D_id);
}

bool PhotometricBundleAdjustmentConfig::HasPoint(const point3D_t point3D_id) const {
  return HasVariablePoint(point3D_id) || HasConstantPoint(point3D_id);
}

bool PhotometricBundleAdjustmentConfig::HasVariablePoint(
    const point3D_t point3D_id) const {
  return variable_point3D_ids_.find(point3D_id) != variable_point3D_ids_.end();
}

bool PhotometricBundleAdjustmentConfig::HasConstantPoint(
    const point3D_t point3D_id) const {
  return constant_point3D_ids_.find(point3D_id) != constant_point3D_ids_.end();
}

void PhotometricBundleAdjustmentConfig::RemoveVariablePoint(const point3D_t point3D_id) {
  variable_point3D_ids_.erase(point3D_id);
}

void PhotometricBundleAdjustmentConfig::RemoveConstantPoint(const point3D_t point3D_id) {
  constant_point3D_ids_.erase(point3D_id);
}

////////////////////////////////////////////////////////////////////////////////
// PhotometricBundleAdjuster
////////////////////////////////////////////////////////////////////////////////

PhotometricBundleAdjuster::PhotometricBundleAdjuster(const PhotometricBundleAdjustmentOptions& options,
                               const PhotometricBundleAdjustmentConfig& config)
    : options_(options), config_(config) {
  CHECK(options_.Check());
}

bool PhotometricBundleAdjuster::Solve(Reconstruction* reconstruction) {
  CHECK_NOTNULL(reconstruction);
  CHECK(!problem_) << "Cannot use the same PhotometricBundleAdjuster multiple times";

  problem_.reset(new ceres::Problem());
  ceres::LossFunction* loss_function = options_.CreateLossFunction();

  /*Do PBA initialized with the points from SfM*/
...
//  solver_options.num_threads = 1;
  ...
  if (options_.print_summary) {
    ...
    PBAPrintSolverSummary(summary_);
  }
...
const ceres::Solver::Summary& PhotometricBundleAdjuster::Summary() const {
  return summary_;
}

void PhotometricBundleAdjuster::SetUp(Reconstruction* reconstruction,
                           ceres::LossFunction* loss_function) {
  /*create photometric landmarks from structure points*/
  CreatePhotometricLandmarksFromGeometricLandmarks(reconstruction);

  AddImageToProblem(reconstruction, loss_function);

  //TODO: add compute_photometric_proejctions and remove_outlier_landmarks_pba
//  compute_photometric_proejctions();
//  remove_outlier_landmarks_pba();
...
void PhotometricBundleAdjuster::TearDown(Reconstruction*) {
  // Nothing to do
}

void PhotometricBundleAdjuster::AddImageToProblem(
                                       Reconstruction* reconstruction,
                                       ceres::LossFunction* loss_function) {
  for (auto& pair : reconstruction->PointsInvD()) {
    auto pointId = pair.first;
    auto& pointInvD = reconstruction->PointInvD(pointId);
    auto& intensityPatch = pointInvD.IntensityPatch();

    auto& hostFrame = pointInvD.HostFrame();
    auto& image_id_i = hostFrame.image_id;

    auto& image_i = reconstruction->Image(image_id_i);
    auto& camera_i = reconstruction->Camera(image_i.CameraId());

    image_i.NormalizeQvec();

    double* qvec_data_i = image_i.Qvec().data();
    double* tvec_data_i = image_i.Tvec().data();

    const bool constant_pose_i =
        !options_.refine_extrinsics || config_.HasConstantPose(image_id_i);

    size_t num_observations=0;
    static double constC=100;
    for (const auto& track : pointInvD.Track().Elements()) {
      auto image_id_j = track.image_id;
      auto pt2DId = track.point2D_idx;  // may not needed

      auto& image_j = reconstruction->Image(image_id_j);
      auto& camera_j = reconstruction->Camera(image_j.CameraId());

      image_j.NormalizeQvec();

      double* qvec_data_j = image_j.Qvec().data();
      double* tvec_data_j = image_j.Tvec().data();

      if (camera_i.ModelId() != camera_j.ModelId()) {
        throw std::runtime_error(
            "two cameras must have the same camera model.");
      }

      const bool constant_pose_j =
          !options_.refine_extrinsics || config_.HasConstantPose(image_id_j);

      auto& interpolator = image_j.IntensityInterpolator();
      Eigen::Vector2i imageBoundary(image_j.ImageWidth(),
                                    image_j.ImageHeight());

      ceres::CostFunction* cost_function = nullptr;

      if (constant_pose_i && constant_pose_j) {
        switch (camera_j.ModelId()) {
#define CAMERA_MODEL_CASE(CameraModel)                                        \
          case CameraModel::kModelId:                                         \
          {                                                                 \
            if (image_i.CameraId() == image_j.CameraId()) {                     \
                cost_function = PhotometricBundleAdjustmentConstantCostFunction<CameraModel>::CreateOneCam(image_i.Qvec(), image_i.Tvec(),image_j.Qvec(), image_j.Tvec(),  \
                  intensityPatch, interpolator,imageBoundary,constC);                  \
            } else {                                                          \
              cost_function = PhotometricBundleAdjustmentConstantCostFunction<CameraModel>::CreateTwoCams(image_i.Qvec(), image_i.Tvec(),image_j.Qvec(), image_j.Tvec(),  \
                intensityPatch, interpolator,imageBoundary,constC);                  \
            }                                                                 \
          break;                                                              \
          }                                                                   \
...
#undef CAMERA_MODEL_CASE
        ...
        if (image_i.CameraId()==image_j.CameraId()) {
          problem_->AddResidualBlock(
              ...
              camera_i.ParamsData(),
              image_i.AffineAB().data(), image_j.AffineAB().data(),
              &pointInvD.InverseDepth());
        } else {
          problem_->AddResidualBlock(
              cost_function, loss_function,
              camera_i.ParamsData(), camera_j.ParamsData(),
              image_i.AffineAB().data(), image_j.AffineAB().data(),
              &pointInvD.InverseDepth());
        }
      } else if (constant_pose_i && !constant_pose_j) {
        switch (camera_j.ModelId()) {
#define CAMERA_MODEL_CASE(CameraModel)                                      \
          case CameraModel::kModelId:                                       \
          {                                                               \
            if (image_i.CameraId()==image_j.CameraId()) {                   \
                cost_function = PhotometricBundleAdjustmentConstantSrcCostFunction<CameraModel>::CreateOneCam( \
                image_i.Qvec(),image_i.Tvec(),                              \
                intensityPatch, interpolator,imageBoundary,constC);                \
            } else {                                                        \
                cost_function = PhotometricBundleAdjustmentConstantSrcCostFunction<CameraModel>::CreateTwoCams( \
                image_i.Qvec(),image_i.Tvec(),                              \
                intensityPatch, interpolator,imageBoundary,constC);                \
            }                                                               \
            break; \
...
#define CAMERA_MODEL_CASE(CameraModel)                                      \
          case CameraModel::kModelId: {                                     \
            {                                                                \
              if (image_i.CameraId() == image_j.CameraId()) {                   \
                cost_function = PhotometricBundleAdjustmentConstantTgtCostFunction<CameraModel>::CreateOneCam(image_j.Qvec(), image_j.Tvec(),                              \
                  intensityPatch, interpolator,imageBoundary,constC);                \
              } else {                                                        \
                cost_function = PhotometricBundleAdjustmentConstantTgtCostFunction<CameraModel>::CreateTwoCams(image_j.Qvec(), image_j.Tvec(),                              \
                  intensityPatch, interpolator,imageBoundary,constC);                \
              }                                                               \
            }\
          } \
...
#define CAMERA_MODEL_CASE(CameraModel)                                    \
          ...
          {                                                              \
            if (image_i.CameraId() == image_j.CameraId()) {               \
              cost_function = PhotometricBundleAdjustmentCostFunction<CameraModel>::CreateOneCam( \
                intensityPatch, interpolator, imageBoundary,constC);              \
            } else {                                                      \
              cost_function = PhotometricBundleAdjustmentCostFunction<CameraModel>::CreateTwoCams( \
                intensityPatch, interpolator, imageBoundary,constC);              \
            }                                                             \
          break;                                                          \
          } \
...
#undef CAMERA_MODEL_CASE
        ...
        if (image_i.CameraId()==image_j.CameraId()) {
          problem_->AddResidualBlock(
              cost_function, loss_function,
              qvec_data_i, tvec_data_i, qvec_data_j, tvec_data_j,
              camera_i.ParamsData(),
              image_i.AffineAB().data(), image_j.AffineAB().data(),
              &pointInvD.InverseDepth());
        } else {
      ...
      // fix tgt if needed
      {
        camera_ids_.insert(image_j.CameraId());

        // Set pose parameterization.
        if (!constant_pose_j) {
          SetQuaternionManifold(problem_.get(), qvec_data_j);
          if (config_.HasConstantTvec(image_id_j)) {
            const std::vector<int>& constant_tvec_idxs =
                config_.ConstantTvec(image_id_j);
            SetSubsetManifold(3, constant_tvec_idxs, problem_.get(), tvec_data_j);
          }
        }
      }
    }
    ...
    if (num_observations > 0) {
      camera_ids_.insert(image_i.CameraId());

      // Set pose parameterization.
      if (!constant_pose_i) {
        SetQuaternionManifold(problem_.get(), qvec_data_i);
        if (config_.HasConstantTvec(image_id_i)) {
          const std::vector<int>& constant_tvec_idxs =
              config_.ConstantTvec(image_id_i);
          SetSubsetManifold(3, constant_tvec_idxs, problem_.get(), tvec_data_i);
        }
      }
    }
  }
...
void PhotometricBundleAdjuster::ParameterizeCameras(Reconstruction* reconstruction) {
...
void PhotometricBundleAdjuster::ParameterizePoints(Reconstruction* reconstruction) {
...
void PhotometricBundleAdjuster::
    CreatePhotometricLandmarksFromGeometricLandmarks(Reconstruction* reconstruction) {
  for (const auto &pair: reconstruction->Points3D()) {
    auto point3dId = pair.first;
    auto &point3D  = pair.second;

    const auto &track = point3D.Track();
    if(track.Length()<2) continue;

    // only create maximum one landmark per point
//    bool created = false;
...
//      }
      ...
      /*project point to image coord*/
      auto image_id = track_el.image_id;
      ...
      /*project*/
      auto qvec_data = image.Qvec().data();
      auto tvec = image.Tvec();
      Eigen::Vector3d point3D_host;
      ceres::UnitQuaternionRotatePoint(qvec_data,point3D.XYZ().data(),point3D_host.data());
      point3D_host += tvec;

      //inverse depth
      auto inverseD = 1.0/point3D_host.z();//1.0 / point3D_host.norm();//TODO: not sure if this should be norm or just z()?

      // Skip if z is not reasonable
      if(inverseD<1e-3 || inverseD > 5) continue;


      const Point2D& point2D_host = image.Point2D(track_el.point2D_idx);

      // get patch information
      IntensityPatch intensityPatch;
      if(!GetPatchInfo(image,point2D_host,intensityPatch)) continue;

      /*add tracks*/
      // skip host
      Track track_pba;
      for (const auto &track_rhs : track.Elements()) {
        if(track_el.image_id == track_rhs.image_id) continue;
        track_pba.AddElement(track_rhs);
      }

      /*create pointinvD*/
      reconstruction->AddPointInvD(inverseD,track_el,track_pba,intensityPatch);
      break;
    }
  }
}
...
////////////////////////////////////////////////////////////////////////////////
// ParallelPhotometricBundleAdjuster
////////////////////////////////////////////////////////////////////////////////

bool ParallelPhotometricBundleAdjuster::Options::Check() const {
  CHECK_OPTION_GE(max_num_iterations, 0);
  return true;
}

ParallelPhotometricBundleAdjuster::ParallelPhotometricBundleAdjuster(
    const Options& options, const PhotometricBundleAdjustmentOptions& ba_options,
    const PhotometricBundleAdjustmentConfig& config)
    : options_(options),
...
bool ParallelPhotometricBundleAdjuster::Solve(Reconstruction* reconstruction) {
  CHECK_NOTNULL(reconstruction);
  CHECK_EQ(num_measurements_, 0)
      << "Cannot use the same ParallelPhotometricBundleAdjuster multiple times";
...
const ceres::Solver::Summary& ParallelPhotometricBundleAdjuster::Summary() const {
  return summary_;
}

bool ParallelPhotometricBundleAdjuster::IsSupported(const PhotometricBundleAdjustmentOptions& options,
...
void ParallelPhotometricBundleAdjuster::SetUp(Reconstruction* reconstruction) {
...
void ParallelPhotometricBundleAdjuster::TearDown(Reconstruction* reconstruction) {
...
void ParallelPhotometricBundleAdjuster::AddImagesToProblem(
    Reconstruction* reconstruction) {
...
void ParallelPhotometricBundleAdjuster::AddPointsToProblem(
    Reconstruction* reconstruction) {
...
////////////////////////////////////////////////////////////////////////////////
// RigPhotometricBundleAdjuster
////////////////////////////////////////////////////////////////////////////////

RigPhotometricBundleAdjuster::RigPhotometricBundleAdjuster(const PhotometricBundleAdjustmentOptions& options,
                                     const Options& rig_options,
                                     const PhotometricBundleAdjustmentConfig& config)
    : PhotometricBundleAdjuster(options, config), rig_options_(rig_options) {}

bool RigPhotometricBundleAdjuster::Solve(Reconstruction* reconstruction,
...
void RigPhotometricBundleAdjuster::SetUp(Reconstruction* reconstruction,
                              std::vector<CameraRig>* camera_rigs,
                              ceres::LossFunction* loss_function) {
...
void RigPhotometricBundleAdjuster::TearDown(Reconstruction* reconstruction,
...
void RigPhotometricBundleAdjuster::AddImageToProblem(const image_t image_id,
                                          Reconstruction* reconstruction,
                                          std::vector<CameraRig>* camera_rigs,
                                          ceres::LossFunction* loss_function) {
...
void RigPhotometricBundleAdjuster::AddPointToProblem(const point3D_t point3D_id,
                                          Reconstruction* reconstruction,
                                          ceres::LossFunction* loss_function) {
  Point3D& point3D = reconstruction->Point3D(point3D_id);
...
void RigPhotometricBundleAdjuster::ComputeCameraRigPoses(
    const Reconstruction& reconstruction,
    const std::vector<CameraRig>& camera_rigs) {
...
void RigPhotometricBundleAdjuster::ParameterizeCameraRigs(Reconstruction* reconstruction) {
